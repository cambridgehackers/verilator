...
static bool jjdumpPtrs(std::ostream& os, AstNode *me, string indent)
{
    indent = "";
    string tname = me->typeName();
    string mname = decodeName(me->name());
    if (AstVar *vn = VN_CAST(me, Var)) {
        std::string descr;
...
zzendl:;
    ...
    if (AstCell* cellp = VN_CAST(me, Cell)) {
...
static void jjdumpTree(std::ostream& os, AstNode *me, const string& indent)
{
